...
- set functions don't update state immediately (cover later) [React may batch multiple setState calls together for performance reasons, but this batching is an implementation detail and should not be relied upon for synchronization or timing purposes.]
...
The reason useEffect should be used at the top level of the component or after all the conditional statements but before any other return statement is to ensure that it is always executed consistently and in the correct order.

    Consistent Execution: Placing useEffect at the top level ensures that it is always executed when the component mounts or updates. This is important for side effects like data fetching, subscriptions, or DOM manipulations.

    Correct Order: By placing useEffect before any other return statement, you ensure that it is executed before the component returns JSX. This is important because the effects may set up the necessary state or perform actions that are required for rendering the component correctly.
...
const UseRefBasics = () => {
  ...
  // refContainer gets updated after initial render 
  useEffect(() => {
...
- When the parent's element re-renders, even if the component's state or props have not changed.

- lower state i.e, now the parent's child component will re-render not child element
